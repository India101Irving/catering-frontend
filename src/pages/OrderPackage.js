...
const DEFAULT_PACKAGES = [
  { id:'pkg-basic', name:'Basic Package', priceLine:'Starting $8/person',
    slots: { appetizer:['A'], main:['A','A'], rice:['A'], bread:['A'], dessert:['A'] }, perPerson:8 },
  { id:'pkg-classic', name:'Classic Package', priceLine:'Starting $12/person',
    slots: { appetizer:['A'], main:['A','A','B'], rice:['B'], bread:['A'], dessert:['A'] }, perPerson:12 },
  { id:'pkg-premium', name:'Premium Package', priceLine:'Starting $15/person',
...
export default function OrderPackage() {
  ...
  const CourseHeaderBadge = ({ course }) => {
    const need = (selection.slots?.[course] || []).length;
    const complete = (picks[course]?.length || 0) === need && need > 0;
    return (
      <span className={`text-xs px-2 py-0.5 rounded-full border ${
        complete ? 'border-green-500/60 text-green-300' : 'border-orange-400/60 text-orange-300'
      }`}>
        {complete ? '✓ Completed' : `Select ${numToWord(need)}`}
      </span>
    );
  };
    ...
    <div className="min-h-screen bg-[#1c1b1b] text-white p-4 md:p-6 md:pr-[24rem] relative">
      ...
      <div className="hidden md:flex absolute top-4 right-[24rem] items-center gap-4 text-sm">
        {currentUser ? (
          ...
          <button onClick={() => nav('/signin', { state: { returnTo: '/OrderTrays' } })}
